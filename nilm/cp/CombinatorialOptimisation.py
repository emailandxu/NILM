...
class CombinatorialOptimisation(Disaggregator):
    ...
    # 训练部分，传入的变量就不一一解释了，英文基础薄弱的可以用谷歌翻译以下。
    ...
    def train(self, metergroup, num_states_dict=None, **load_kwargs):
        ...
        * only uses first chunk for each meter (TODO: handle all chunks).
        ...
        if num_meters > 12:  # 如果num_meters > 12,那么聚类中心的个数就是2，否则为3，聚类这部分在后面再解释
            ...
            max_num_clusters = 3
        # metersgroup是传进来的变量，metergroup就是将多种电器的消耗功率数据封装成group了，通过调用group下的函数如meters()和submeters()可以调出单个电器的独立历史数据，这个循环主要就是用来遍历group中的电器的种类
        for i, meter in enumerate(metergroup.submeters().meters):
            print("Training model for submeter '{}'".format(meter))
            # meter.power_series()可以返回pandas格式的数据，这里得到的是一个生成器，power_series()中可如参数如sample_period = 6,即采样周期是6s
            ...
            if num_total_states is not None:
                ...
                num_on_states = None
            self.train_on_chunk(
                chunk, meter, max_num_clusters, num_on_states)  # 批训练
    ...
    def train_on_chunk(self, chunk, meter, max_num_clusters, num_on_states):  # 这是核心部分
        ...
        # 整合数据集中的一些元数据，元数据包含了该电器的很多信息，如线路信息，型号信息等等。
        ...
        if meter in meters_in_model:
            raise RuntimeError(
                ...
                .format(meter))
        # 这是核心中的核心，下面会将clustering的代码解释一下
