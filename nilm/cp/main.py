...
from .CombinatorialOptimisation import CombinatorialOptimisation
...
def predict(clf, test_elec, sample_period, timezone):  # 定义预测的方法
    ...
    for i, chunk in enumerate(test_elec.mains().load(sample_period=sample_period)):
        ...
        # 分解，disaggregate_chunk是CO下的一个方法，通过调用这个方法实现分解，这部分代码在下面可以见到
        ...
        gt[i] = pd.DataFrame({k: v.squeeze() for k, v in iteritems(gt[i])}, index=next(
            iter(gt[i].values())).index).dropna()  # 上面这一块主要是为了得到pandas格式的gt数据
...
for clf_name, clf in classifiers.items():
    ...
    gt, predictions[clf_name] = predict(
        clf, test_elec, 120, train.metadata['timezone'])  # 预测和分解
...
def compute_rmse(gt, pred):  # 评估指标 rmse
    ...
    for appliance in gt.columns:
        rms_error[appliance] = np.sqrt(mean_squared_error(
            gt[appliance], pred[appliance]))  # 评价指标的定义很简单，就是均方根误差
